...
function getTodaySlots(slots, config, isToday) {
	const hourly = slots.filter((s) => s.time.endsWith(":00") && s.time >= config.wake_time && s.time < config.sleep_time);
	if (!isToday) return hourly;
...
*/
var NEXT_DAY_MAX = 6;
function getNextDaySlots(slots, config) {
	const hourly = slots.filter((s) => s.time.endsWith(":00") && s.time >= config.wake_time && s.time < config.sleep_time);
	let lastMomentIdx = -1;
	hourly.forEach((s, i) => {
...
function DailyGrid({ day, nextDay, config, onToggleMoment, nextMomentKey }) {
	...
	}), showNextDay && nextDay && /* @__PURE__ */ jsxs("section", {
		className: "daily-grid daily-grid--next-day",
		children: [/* @__PURE__ */ jsxs("header", {
			className: "daily-grid__header",
			children: [/* @__PURE__ */ jsx("span", {
				className: "daily-grid__day-name",
				children: nextDay.dayName
			}), /* @__PURE__ */ jsx("span", {
				className: "daily-grid__date",
				children: format(parseISO(nextDay.date), "d MMMM yyyy")
			})]
		}), /* @__PURE__ */ jsx("div", {
			className: "daily-grid__slots daily-grid__slots--next-day",
